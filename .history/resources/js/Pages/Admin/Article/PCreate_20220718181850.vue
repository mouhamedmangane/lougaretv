<template>
    <app-admin-layout class="bg-neutral-100">
        ...
        <admin-contained-box class="border mt-4 mx-4 yyy"  >
            <!-- content -->
            <div  class="bg-white" >
                <article-create  :article="article"  ref="formCreate" redirect="/lougare_npl/article" :mode="mode" />
            </div>
            <div >
            </div>
        </admin-contained-box>
...
import ArticleCreate from '@/MyComponent/Article/ArticleCreate.vue'
import BreadcumbItem from '@/MyComponent/Breadcumb/BreadcumbItem.vue'

export default {
  components: { AppAdminLayout, AdminContainedBox, Breadcumb, BreadcumbItemFinal, SimpleButton, ArticleCreate, BreadcumbItem},
  props:['article','mode'],
  computed:{
      titre(){
          return (this.mode=="create")? 'Nouveau':this.article.name;
      }
  },
