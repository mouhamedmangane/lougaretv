    ...
    <app-admin-layout class="bg-neutral-100">
        ...
        <div class="bg-neutral-100 mx-4 px-4 border w-full" >
            <div class="flex justify-between border-y w-full py-2 ">
                ...
                <div class="" v-if="operation.length">
                    <div class="flex justify-end items-center mr-8">
                        <span class="mr-4 text-primary-600">{{operation}}</span>
                        <spinner />
                    </div>
                </div>
...
export default {
  ...
  data(){
      return {
          ...
          limit:25,
          liste:[],
          columns:['N°','Rédacteur','Titre','D.Crée','D.Modifié','Action'],
      }
  },
  created(){
      this.getListe();
  ...
  methods:{
      ...
      dropCase(event){
          if(event.dragIndex>-1){
              let temp=this.alaunes[event.dragIndex];
              this.alaunes[event.dropIndex]=this.alaunes[event.dragIndex];
              this.alaunes[event.dragIndex]=temp;
      ...
      save(){
          this.operation="Enrgistement A la une";

          axios.post('/lougare_npl/articles/storeAlaune',{'articleIds':this.dataAlaune()})
          .then(res=>{
              ...
              alert("Impossible d'envoyér les données, Verifier votre connexion ");
              this.operation='';
