<template>
    <app-admin-layout class="bg-neutral-100">
        <admin-contained-box class="mx-4  bg-neutral-100 border">
            <!-- titre et toolbar -->
            ...
            <div class="flex justify-between border-y w-full py-2 ">
                <div class="flex" >
                    <simple-button icon="save" txt="Enregistrerz"
                            ...
                            @click="onClickSave"
                     />
                     <simple-button icon="delete" txt="Supprimer" :disabled="(mode=='edit')?false:'disabled'"
                            class="mx-2 disabled:text-neutral-500 enabled:hover:bg-neutral-300" @click="onClickDelete"
                     />

                </div>

                <div class="flex justify-end">
                    <simple-lien icon="add" txt="Nouveau" url="/lougare_npl/article/create"
                            class="mx-2  hover:bg-neutral-300"
                     />
                </div>
            </div>
        </div>
...
export default {
  components: { AppAdminLayout, AdminContainedBox, Breadcumb, BreadcumbItemFinal, SimpleButton, BreadcumbItem, ArticleAdminCreate, SimpleLien},
  ...
  methods:{
      ...
      onClickDelete(){
          let ok=confirm("Etes vous sûre de vouloir supprimer l'article: N°"+this.article.id);
          if(ok){
                axios.delete('/lougare_npl/article/'+this.article.id)
                .then(res=>{
                    if(res.data.status){
