                ...
                <table class="table-auto w-full">
                    ...
                    <tbody>
                        <tr v-for="item in liste" :key="item.id" class="py-1 even:bg-neutral-200 group">
                            ...
                            <td class="p-4">
                                <div class="">
                                    <img :src="'/images/redacteurs/'+item.image" alt="" class="w-12 h-12 rounded-full">
                                </div>
                            </td>
                            <td class="p-4">
...
export default {
  ...
  data(){
      return {
          search:"",
          liste:[],
          columns:['N°','Image','Nom','Email','Date','Action'],
      }
  },
  created(){
      this.getListe();
  ...
  methods:{
      ...
      deleteRedacteur(item){
          let ok=confirm("Etes vous sûre de vouloir supprimer la redacteur : "+item.name.toUpperCase());
          if(ok){
            axios.delete('/lougare_npl/redacteur/'+item.id)
            .then(res=>{
                if(res.data.status){
                    this.getListe();
                }
