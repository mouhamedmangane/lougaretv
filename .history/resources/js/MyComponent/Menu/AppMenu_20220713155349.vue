<template>
    ...
    <div :class="{ 'w-full  sticky top-0    z-20   transition-all   ':true,'text-black bg-white shadow-lg   ':isSmall,'  bg-[#7D5241] border-b  border-neutral-300':!isSmall}">
        <container-box :class="{'flex justify-between container mx-auto  items-center transition-all relative   border-neutral-300':true}">
             <div :class="{'flex items-center   px-4 py-1 absolute  left-0  h-[54.5px]':true,' top-1  border-l  border-neutral-300':!isSmall,'top-0':isSmall} " style="border-radius:20px 0px 0px 20px;border-top-color:transparent;">
                    <a href="/" class=" ">
            ...
            <nav :class=" {'py-2':isSmall,'py-3':!isSmall,'':true}">
            ...
            <div class="flex justify-end items-center ">
                <material-icon-text  class="ml-2 mr-4 bg-red-700 rounded-full px-4" materialicon='play_circle_filled' text="en direct" iconClass="text-2xl mr-1 text-white" textClass="font-medium text-xl  text-white"/>
                <material-icon nom="search" class="text-2xl ml-2 " />
                <!-- <material-icon nom="notifications" type="outlined" class="text-2xl ml-2 " /> -->
                <button class="ml-2   rounded-full w-10 h-10  rounded-full border-black  border-2 ">
