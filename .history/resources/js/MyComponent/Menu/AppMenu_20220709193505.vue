<template>
    ...
    <div :class="{ 'w-full  sticky top-0    z-20   transition-all   border-b':true,'text-black bg-white shadow-lg   ':isSmall,'   border-b':!isSmall}">
        <container-box :class="{'flex justify-between container mx-auto  items-center transition-all relative   border-neutral-300':true}">
             <div :class="{'flex items-center  bg-white px-4 py-1 absolute   left-0  h-[54.5px]':true,' border-x top-1':!isSmall,'top-0':isSmall} ">
                    <a href="/" class=" ">
            ...
            <nav :class=" {'py-2':isSmall,'py-3':!isSmall,'flex items-center':true}">
                <h2 class="text-masquer">Menu</h2>
                <ul class="list-none flex items-center font-medium text-[16px]">
                    <menu-item v-for="text in elements"  :text="text"  class="leading-none"/>
                    <!-- <li><more-item class="hover:underline " /></li> -->
                </ul>
            </nav>

            <div class="flex justify-end items-center ">
                <material-icon-text  class="ml-2 mr-4 leading-none" materialicon='play_circle_filled' text="En direct" iconClass="text-2xl mr-1 text-gray-900" textClass="font-medium text-xl "/>
                <material-icon nom="search" class="text-2xl ml-2 leading-none" />
                <!-- <material-icon nom="notifications" type="outlined" class="text-2xl ml-2 " /> -->
                <button class="ml-2  flex items-center rounded-full ">
                    <material-icon nom="person" type="outlined" class="text-2xl leading-none" />
                </button>
