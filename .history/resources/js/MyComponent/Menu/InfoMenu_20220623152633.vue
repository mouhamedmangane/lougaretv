  ...
  <div class="flex py-2 px-4 items-center">
    ...
    <more-item v-if="canMore" >
        <ul class="list-none text-black bg-white border">
            <li v-for="item in itemMasquer" :class="{'p-2 px-3 shadow':true,'bg-white text-black':isActiveItem(item)}">{{item}}</li>
        </ul>
    </more-item>
...
export default {
  ...
  methods:{
      isActiveItem(item){
        return (this.active)? this.active==item:false;
      }
  }
