<template>
    ...
    <div :class="{ 'w-full  md:sticky top-0    z-20   transition-all   ':true,'text-black bg-white shadow-lg   ':isSmall,' text-black bg-white shadow-lg   border-neutral-300':!isSmall}">
        <container-box :class="{'relative flex justify-between container mx-auto  items-center transition-all relative   border-neutral-300':true}">
             <div :class="{'flex items-center   px-4 md:py-1 absolute  left-0  h-[54.5px]':true,' md:top-1    border-neutral-300':!isSmall,'top-0':isSmall} " style="">
                    <a href="/" class=" ">
            ...
            <nav :class=" {'md:py-3':isSmall,'md:py-3':!isSmall,'md;py-3 absolute md:static top-[100%] left-0  w-full md:w-fit  overflow-hidden transition-all bg-red-100 md:bg-transparent z-50':true,'h-[100vh - 40px] md:h-fit':isDerouler,'h-0 md:h-fit':!isDerouler}">
                <h2 class="text-masquer">Menu</h2>
                <ul class="list-none flex flex-col md:flex-row  font-medium text-[15px]">
                    <menu-item text="A la une" class=" border-white"/>
