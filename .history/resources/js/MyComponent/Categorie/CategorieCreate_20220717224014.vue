<template>
    ...
    <form action="" class=" p-4 grid grid-cols-2 gap-4" :id="idForm"  @submit="onSubmit($event)" method="post">
...
export default {
  components: { ChoixCouleur, SimpleButton },
    props:['categorie',"bus"],
    methods:{
        onSubmit(event){
            console.log('form-create-categorie');
            event.preventDefault();
            axios.post('/lougare_npl/categorie/store',this.categorie
            ).then(res=>{
    ...
    mounted(){
        this.bus.$on('submit',this.onSubmit);
    }
}
</script>
