<template>
    ...
    <form action="" class=" p-4 grid grid-cols-2 gap-4" @submit="onSubmit" method="post" ref="myForm">
        ...
        <div class="col-start-2">
            <div class="flex items-center">
                <input-image v-model="redacteur.photo" :pathImage="'/images/redacteurs/'+redacteur.image"
        ...
        <div class="col-start-2">
            <input type="text" v-model="redacteur.name"  :class="{'':true,'border-red-600':messages.name}" @keyup="messages.name=false">
        ...
        <div class="col-start-2">
            <input type="text" v-model="redacteur.email" :class="{'':true,'border-red-600':messages.email}" @keyup="messages.email=false">
        ...
        <div class="col-start-2">
            <input type="text" v-model="redacteur.pwd" :class="{'':true,'border-red-600':messages.pwd}" @keyup="messagse.pwd=false">
  ...
  components: { ChoixCouleur, SimpleButton, MessageInput, InputImage },
    ...
    methods:{
        ...
        onSubmit(event){
            ...
            let redacteur=_.cloneDeep(this.redacteur)
            redacteur=(redacteur)?redacteur:this.redacteur;
            console.log(redacteur);
            for(let key in redacteur){
                console.log(key);
                formdata.append(key,this.redacteur[key])
            }
            ...
            if(this.mode=="create"){
                ...
                axios.post('/lougare_npl/redacteur/'+this.redacteur.id,
                    formdata,
                    {
