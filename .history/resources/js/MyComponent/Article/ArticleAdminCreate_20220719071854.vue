<template>
    <div class="mx-2 bg-white p-4 flex justify-between shadow-md" >
        <form action="" class=" p-4 grid  gap-4" @submit="onSubmit" method="post" ref="myForm">
            ...
            <div class="col-start-2">
                <textarea type="text" v-model="article.resume" :class="{'2xl:w-80':true,'border-red-600':messages.resume}" @keyup="messages.resume=false" rows="3"></textarea>
                <message-input :messageItem="messages.resume" :status="status"  />
            </div>
        ...
        onSubmit(event){
            ...
            for(let key in article){
                ...
                if(key!='elements'){
                    ...
                    for(let element of _.cloneDeep(this.article[key])){
                        for(let modelKey in element.mmodel){
                            //formdata.append('elements[]',JSON.parse(JSON.stringify(element)));
                            if(element.mmodel.$file)
                                formdata.append('files['+element.mmodel.position+']',element.mmodel.$file);
                            delete element.mmodel.$file;
                            delete element.mmodel.src;
                            console.log(modelKey);
                            formdata.append('elements['+element.mmodel.position+']',JSON.stringify(element));
