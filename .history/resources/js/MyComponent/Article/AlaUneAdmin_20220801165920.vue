<template>
    <div class="flex flex-wrap justify-center w-full py-4 2xl:pt-0 2xl:w-[32rem] mr-4">
        <div class="relative border relative  w-64 2xl:w-1/2  h-56 2xl:h-46   bg-white group"
             v-for="i in [0,1,2,3]"
            ...
            <button class="absolute bottom-0 right-0 py-1 px-2  border d-flex items-center z-20 bg-[rgba(200,0,0,0.3)] hover:bg-red-600 text-white hidden group-hover:inline-block"
                 @click="deleteAlaUne(i)"
                 v-if="!alaunes[i]">
...
export default {
  components: { MaterialIcon },
    ...
    methods:{
      ...
      deleteAlaUne(index){
        this.$emit('delAlaune',{dropIndex:i,dragIndex:this.dragIndex});
