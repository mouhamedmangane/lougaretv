<template>
    <div class="group  relative bg-gray-200" @click="onClick">
        <my-img :src="pub.image" alt="" :class="{'order-2':inverser,'order-1 float-left relative':!inverser} " :size="imageSize" />
        <div :class=" {'order-1 flex flex-col justify-between max-h-24 relative':true,']pr-2':inverser,'pl-2':!inverser}">
            <titre-3 :size='textSize' class="group-hover:underline  grow'">
                <text-limit :text="pub.titre" :maxLength="limitTitre" />
            </titre-3>
            <div class=" text-xs text-neutral-600 pb-1 absolute bottom-0" >societé</div>
