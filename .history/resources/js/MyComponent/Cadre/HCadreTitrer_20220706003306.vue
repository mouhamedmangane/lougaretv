<template>
    <div class="group  h-full" @click="onClick">
        <my-img :src="pub.image" alt="" :class="{'float-right':inverser,' float-left ':!inverser,'relative shadow inline':true} " :size="imageSize" />
        <div :class=" {'relative  h-full ':true,']pr-2':inverser,'pl-2':!inverser}">
            <titre-3 :size='textSize' class="group-hover:underline  inline">
                <text-limit :text="pub.titre" :maxLength="limitTitre" />
            </titre-3>
            <div class="px-2 rounded-full text-xs absolute bottom-0 inline" >societé</div>
